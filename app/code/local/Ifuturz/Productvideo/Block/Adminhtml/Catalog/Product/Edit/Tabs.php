...
class Ifuturz_Productvideo_Block_Adminhtml_Catalog_Product_Edit_Tabs
...
echo $this->getLayout()->createBlock($this->getAttributeTabBlock());
			...
			if (Mage::helper('core')->isModuleEnabled('Mage_CatalogInventory')) 
			{
				...
				/* ------------------- START Added code by Rifuturz ----------------------- */
				if($geteditid != '')
				{
					$this->addTab('Product Video', array(
						'label'     => Mage::helper('catalog')->__('Product Video'),
						'active'	=> true,
						'content'   => $this->_translateHtml($this->getLayout()
							->createBlock('productvideo/adminhtml_catalog_product_edit_tab_productvideo')->toHtml()),
					));
				}
				else
				{
